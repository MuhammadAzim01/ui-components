...
async function program_container (opts, invite) {
  ...
  if (invite) {
    ...
    _.up && _.up.send('set_doc_display_handler', { callback: on_doc_display }, {})
  }
  ...
  function on_doc_display (display_data) {
    ...
    _.docs_window.send('display_doc', { content, sid })
  }
  ...
  function console_history_protocol (msg) { _.up && _.up.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} ) }
  ...
  function actions_protocol (msg) {
    ...
    function actions_forward_up (msg) { _.up && _.up.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} ) }
  }

  function actions_selected_action (msg) {
    const { data } = msg
    _.up && _.up.send('update_quick_actions_input', data, msg.head ? { cause: msg.head } : {} )
  }

  function actions_ui_focus_docs (msg) { _.up && _.up.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} ) }

  function tabbed_editor_protocol (msg) { _.up && _.up.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} ) }

  function graph_explorer_protocol (msg) { _.up && _.up.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} ) }
  ...
  function onmessage (msg) {
    ...
    function onmessage_filter_actions (msg) { _.actions.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} ) }
    function onmessage_tab_close_clicked (msg) { _.tabbed_editor.send('close_tab', msg.data, msg.head ? { cause: msg.head } : {} ) }
    function onmessage_entry_toggled (msg) { _.graph_explorer.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} ) }
    function onmessage_execute_step (msg) {
      ...
      update_program_layout()
      _.graph_explorer.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} )
    }
    function onmessage_send_actions (msg) { _.actions.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} ) }
    function onmessage_tab_name_clicked (msg) {
      tabbed_editor_toggle_view(true)
      _.tabbed_editor.send('toggle_tab', msg.data, msg.head ? { cause: msg.head } : {} )
    }
    function onmessage_switch_tab (msg) {
      tabbed_editor_toggle_view(true)
      _.tabbed_editor.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} )
    }
    function onmessage_display_doc (msg) {
      docs_window_el.classList.remove('hide')
      _.docs_window.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} )
    }
  }
}
