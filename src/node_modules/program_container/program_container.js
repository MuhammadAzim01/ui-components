...
async function program_container (opts, protocol) {
  ...
  function resolve_display_state (display_data, current_view) {
    ...
    if (display_data && typeof display_data === 'object' && display_data.display !== undefined) return display_data.display !== 'none'
    return !current_view
  }
  ...
  function update_program_layout () {
    ...
    const primary_visible = tabbed_visible || graph_visible
    const actions_visible = !actions_el.classList.contains('hide')
    const console_visible = !console_history_el.classList.contains('hide')
    const visible_count = (primary_visible ? 1 : 0) + (actions_visible ? 1 : 0) + (console_visible ? 1 : 0)
    ...
    if (primary_visible) {
      if (visible_count === 1) primary_row = '1fr'
      else if (visible_count === 2) primary_row = 'minmax(220px, 2fr)'
      else primary_row = 'minmax(220px, 3fr)'
    }
    if (actions_visible) {
      actions_row = visible_count === 1 ? '1fr' : 'minmax(96px, 1fr)'
    }
    if (console_visible) {
      console_row = visible_count === 1 ? '1fr' : 'minmax(96px, 1fr)'
    }
    if (!primary_visible && !actions_visible && !console_visible) {
      primary_row = '1fr'
    }

    program_main.style.gridTemplateRows = `${primary_row} ${actions_row} ${console_row}`
  }
