...
async function theme_widget (opts, invite) {
  ...
  function onmessage_from_root (msg) {
    ...
    function root_update_actions_for_app (msg) {
      if (_.program_container) _.program_container.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} )
      else setTimeout(root_retry_send_program_container, 500, msg)
    }

    function root_retry_send_program_container (msg) { _.program_container.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} ) }
    function root_forward_taskbar (msg) { _.taskbar.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} ) }
  }

  // Inline focus tracker: handles ui_focus messages from children
  function handle_ui_focus (msg) {
    if (last_focused !== msg.data.type) {
      _.up && _.up.send('focused_app_changed', msg.data, {})
    }
  ...
  function program_container_protocol (msg) {
    ...
    function program_container_forward_up (msg) { _.up && _.up.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} ) }
    function program_container_forward_taskbar (msg) { _.taskbar.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} ) }
  }
  ...
  function taskbar_protocol (msg) {
    ...
    function taskbar_forward_up (msg) { _.up && _.up.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} ) }
    function taskbar_forward_program_container (msg) { _.program_container.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} ) }
    function taskbar_docs_toggle (msg) {
      _.program_container.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} )
    }
  }
}
