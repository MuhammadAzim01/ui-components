...
`net_helper` provides a shared message router for component-to-component communication.
...
- `net_helper` wires both directions and creates `_` channel helpers automatically
...
```js
const head = _.petname(type, refs, data)
```

Each channel helper is a function:
...
The helper returns the generated `head`, so callers can keep it for later response matching:

```js
const head = _.petname('request', {}, data)
```
...
function io_petname () {
  ...
  function handle_response (msg) {
    const head = _.petname('done', { cause: msg.head }, { ok: true })
  }
}
```
...
```js
_.petname('done', { cause: msg.head }, data)
```

Use `{}` for UI/root-originated messages:

```js
_.up('ui_focus', {}, { type: 'wizard_hat', sid: opts.sid })
```
...
`net_helper` forwards automatically based on the recipient in `head`.

That means components using `net_helper` should not add extra manual forwarding just to move a message across already-connected net channels.
