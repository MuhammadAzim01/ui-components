...
async function action_executor (opts, invite) {
  ...
  function program_load_actions (data, type, msg) {
    variables = data
    _.up && _.up.send(type, data, msg.head ? { cause: msg.head } : {} )
  }
  ...
  function steps_wizard_protocol (msg) {
    ...
    else _.up && _.up.send(type, msg.data, msg.head ? { cause: msg.head } : {} )
  }

  function steps_wizard_step_clicked (msg) {
    const { data } = msg
    const refs = msg.head ? { cause: msg.head } : {} 
    _.up && _.up.send('step_clicked', data, refs)

    if (should_execute_step(data)) {
      _.up && _.up.send('execute_step', {
  ...
  function form_action_submitted (data, type, msg) {
    ...
    const refs = msg.head ? { cause: msg.head } : {} 
    _.program.send('update_data', all_data, refs)
    _.steps_wizard.send('init_data', selected_action.steps, refs)

    if (selected_action.steps[selected_action.steps.length - 1]?.is_completed) {
      _.up && _.up.send('show_submit_btn', null, refs)
    }
  }
  ...
  function form_action_incomplete (data, type, msg) {
    ...
    _.program.send('update_data', all_data, refs)
    _.steps_wizard.send('init_data', selected_action.steps, refs)
    _.up && _.up.send('hide_submit_btn', null, refs)
  }
  ...
  function form__action_complete (data, type, msg) {
    ...
    if (all_mandatory_complete) {
      _.up && _.up.send('action_auto_completed', { selected_action, trigger: 'form' }, msg.head ? { cause: msg.head } : {} )
    }
  ...
  function onmessage (msg) {
    const { type } = msg
    if (type === 'docs_toggle') {
      _.steps_wizard.send(type, msg.data, msg.head ? { cause: msg.head } : {} )
      for (const name in component_modules) {
        _[name].send(type, msg.data, msg.head ? { cause: msg.head } : {} )
      }
    } else {
  ...
  function load_actions (msg) {
    variables = msg.data
    _.program.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} )
  }
  ...
  function action_submitted (msg) {
    ...
    _.program.send('display_result', data, msg.head ? { cause: msg.head } : {} )
  }
  ...
  function render_form (msg) {
    ...
    if (send) {
      send.send('step_data', data, msg.head ? { cause: msg.head } : {} )
    }
  }
  ...
  function update_data (msg) {
    ...
    _.program.send(type, msg_data, msg.head ? { cause: msg.head } : {} )
  }
  ...
  function activate_steps_wizard (msg) {
    ...
    _.steps_wizard.send('init_data', data, msg.head ? { cause: msg.head } : {} )
  ...
  function form_data (msg) {
    ...
    _.steps_wizard.send('init_data', msg.data, msg.head ? { cause: msg.head } : {} )
  }
