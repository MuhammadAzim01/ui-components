...
async function tabsbar (opts, invite) {
  ...
  if (dricons[2]) {
    ...
    function on_bar_btn_click () {
      ...
      _.up && _.up.send('set_docs_mode', { active: docs_toggle_active }, {})
      // Also send docs_toggle notification for UI updates
      _.up && _.up.send('docs_toggle', { active: docs_toggle_active }, {})
      bar_btn.classList.toggle('active', docs_toggle_active)
      _.task_manager.send('docs_toggle', { active: docs_toggle_active }, {})
    }
  }
  ...
  function handle_docs_toggle (msg) { _.tabs.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} ) }

  function onmessage_fail () {
    // Handle other message types
  }

  function tabs_protocol (msg) { _.up && _.up.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} ) }

  function task_manager_protocol (msg) { _.up && _.up.send(msg.type, msg.data, msg.head ? { cause: msg.head } : {} ) }
